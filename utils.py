...
def load_models():
    """
    Loads models from the specified in the CONFIG file
    :Input
        model_path - string denoting path to load models from
    ...
    for file in model_file_paths:
        with open(os.path.join(config["model_folder"], file), 'rb') as File:
